...
app.post('/upload', upload.single('audioBlob'), (req, res) => {
  ...
  const fileBuffer = Buffer.from(new Uint8Array(req.file.buffer));
  ...
  fragments.forEach((fragment, idx) => {
    ...
    const learnerFragmentPath = `${basePath}/${timestamp}_${fileSuffix}.mp3`;
    const cmdExtractSection = `ffmpeg -loglevel panic -i ${mp3Path} -ss ${startTime} -to ${endTime} -acodec copy ${learnerFragmentPath}`;
    shell.exec(cmdExtractSection);

    const orignalFragmentPath = `${basePath}/original/word_${fileSuffix}.mp3`;
    const cmdGetScore = `python computeDistance.py ${orignalFragmentPath} ${learnerFragmentPath}`;

    const score = shell.exec(cmdGetScore).stdout;
    fragment.score = score.trim();
  })
